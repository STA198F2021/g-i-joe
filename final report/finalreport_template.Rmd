...
fisher.test(drug_clean_2$Gender, drug_clean_2$Benzos_User)
```
...
```{r x-squared-test-for-country}
#null_distribution_simulated <- drug_clean_2 %>%
  #specify(Country ~ Benzos_User) %>%
...
drug_clean %>%
  ggplot(aes(Benzos_User, Impulsive)) + 
  geom_boxplot() + 
  geom_jitter(width = 0.1, alpha = 0.1)
```
...
fit_multi <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(Benzos_User ~ Gender + Age + Education + Nscore + Oscore + Impulsive + SS, data=drug_clean_2, family="binomial")
